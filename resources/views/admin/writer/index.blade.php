...
<div class="row wrapper border-bottom white-bg page-heading">
	<div class="col-lg-10">
		...
		<ol class="breadcrumb">
			...
			<li class="active"><strong>Writer</strong></li>
		</ol>
	</div>
			...
			<div class="ibox float-e-margins">
				...
				<div class="ibox-content">
					...
					<table class="table table-striped table-bordered table-hover dataTables-example" >
						...
						<tr class="gradeX">
							<td style="width:40px;"><input type="checkbox" name="chkDel[]" id="chkDel{{ $key + 1 }}" value="{{ $writer[$key]['writer_id'] }}"></td>
							<td style="width:40px;" align="center">{{ $writer[$key]['writer_id'] }}</td>
							<td align="center"><img src="{{ URL::asset('media/writer/1.png') }}" alt="{{ $writer[$key]['writer_name'] }}" style="width:80px;"></td>
							<td>{{ $writer[$key]['writer_name'] }}</td>
							<td style="width:150px;">{{ $writer[$key]['writer_surname'] }}</td>
							<td style="width:150px;">{{ $writer[$key]['writer_pen'] }}</td>
							<td style="width:150px;">{{ $writer[$key]['book_count'] }}</td>
							<td style="width:150px;">{{ $writer[$key]['user_name'] }}</td>
							<td align="center" style="width:50px;">
								<a href="{{ URL::to('admin/book/detail') }}"><i class="fa fa-plus-square-o" aria-hidden="true"></i></a>
								<a onclick="editRow('{{ $writer[$key]['writer_name'] }}', '{{ $writer[$key]['writer_surname'] }}', '{{ $writer[$key]['writer_pen'] }}', '{{ $writer[$key]['writer_image'] }}');"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>
...
<!-- Bootbox Edit Row -->
<script>
	function editRow(name, surname, pen, image) {
		$('#name').val(name);
		$('#surname').val(surname);
		$('#pen').val(pen);
		$('#image').val(image);
